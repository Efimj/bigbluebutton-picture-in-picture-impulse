...
import { useParticipants } from '../plugin-pip/components/cameras/hooks';
...
function MainComponent({ pluginUuid }: MainComponentProps): React.ReactNode {
  ...
  const { data: participants } = useParticipants(pluginApi);
  const { data: screenshare } = useScreenshare(pluginApi);
  const hasParticipants = Boolean(participants?.user?.some((user) => !user.bot));
  const hasScreenshare = Boolean(screenshare?.screenshare?.length);
  const hasMedia = hasScreenshare || hasParticipants;
